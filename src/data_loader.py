...
# --- Configuration ---
# Define the root directory for data (best practice)
# NOTE: The path is relative to the directory where the calling script (main.py) is located.
# Since main.py is in src/, we go up one level (..) and then into data/.
...
def load_and_preprocess_data() -> Optional[pd.DataFrame]:
    ...
    # We only care about the cards in the 'data' key.
    ...
    for card_name, details in card_data.items():
        # MTGJSON often has an array for each card name (for split/flip/etc.)
        # We take the first element for simplicity for a single row per card name
        card_details = details[0] if isinstance(details, list) and details else details
        
        # We must skip if the details are somehow invalid
    ...
    # Ensure the column exists and filter for 'Legal'.
    if 'legalities.commander' not in df.columns:
        ...
        # Note: We filter for 'Legal' and drop NaNs just in case.
    ...
    for new_name in final_columns.values():
        if new_name not in final_df.columns:
            # Keywords are lists, everything else is fine with a default empty string
...
def load_collection_data() -> Optional[List[str]]:
    ...
    try:
        # Moxfield CSV typically has 'Count', 'Name', 'Set', etc.
...
if __name__ == "__main__":
    ...
    if card_db is not None:
        ...
        # Displaying the first few rows to confirm success
        print(card_db.head())

        # Example check: Find a common Commander staple
        sol_ring = card_db[card_db['Name'] == 'Sol Ring']
        if not sol_ring.empty:
            print("\n--- Example Query: Sol Ring ---")
            print(sol_ring[['Name', 'ManaValue', 'ColorIdentity', 'Text']].iloc[0])
        else:
            print("\nSol Ring not found (data file may be missing cards or Commander set is banned).")